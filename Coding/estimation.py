...
# Suppress convergence warnings from heavy ensemble iterations
...
models = {
    'OLS - Basic': {
        ...
        'ml_m': make_pipeline(StandardScaler(), LinearRegression()) 
    },
    'L1 (Lasso / Logit L1)': {
        'ml_l': make_pipeline(StandardScaler(), LassoCV(cv=5, random_state=42, max_iter=10000, n_jobs=1)),
        'ml_m': make_pipeline(StandardScaler(), LogisticRegressionCV(cv=5, penalty='l1', solver='liblinear', random_state=42, max_iter=10000, n_jobs=1))
    },
    'L2 (Ridge / Logit L2)': {
        'ml_l': make_pipeline(StandardScaler(), RidgeCV(cv=5)),
        'ml_m': make_pipeline(StandardScaler(), LogisticRegressionCV(cv=5, penalty='l2', solver='lbfgs', random_state=42, max_iter=10000, n_jobs=1))
    },
    'Elastic Net': {
        'ml_l': make_pipeline(StandardScaler(), ElasticNetCV(cv=5, l1_ratio=[0.1, 0.5, 0.9], random_state=42, max_iter=10000, n_jobs=1)),
        'ml_m': make_pipeline(StandardScaler(), LogisticRegressionCV(cv=5, penalty='elasticnet', solver='saga', l1_ratios=[0.1, 0.5, 0.9], random_state=42, max_iter=10000, n_jobs=1))
    },
    'Single Tree': {
        'ml_l': DecisionTreeRegressor(max_depth=5, random_state=42),
        'ml_m': DecisionTreeClassifier(max_depth=5, random_state=42)
    },
    'Random Forest': {
    ...
    'Ensemble (Stacking)': {
        ...
        'ml_m': StackingClassifier(estimators=base_classifiers, final_estimator=make_pipeline(StandardScaler(), LogisticRegressionCV(cv=5, penalty='l2', solver='lbfgs')))
    }
}
...
for model_name, ml_dict in models.items():
    ...
    for policy in core_policies:
        ...
        # Store PLM results as separate raw floats
        row_data['PLM_ATE_coef'] = dml_plr.coef[policy]
        row_data['PLM_ATE_pval'] = dml_plr.pval[policy]
        ...
        try:
            ...
            # Store ATET as separate raw floats
            ...
            # This keeps your CSV columns cleanly formatted as floats
...
for policy in core_policies:
    ...
    # Use standard console print to display the matrices cleanly, matching your previous preference
