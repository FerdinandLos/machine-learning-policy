...
# Suppress sklearn future warnings for clean console output
...
# ---------------------------------------------------------
# OPTIMIZATION: Use static base estimators (no 'CV') for Stacking to prevent nested workload explosion
base_regressors = [
...
base_classifiers = [
    # Using the modern syntax for fixed elasticnet in base classifier
...
# OPTIMIZATION: Inner CV dropped to 3. Max iterations dropped to 2000.
# SYNTAX FIX: Replaced 'penalty' with 'l1_ratios' and applied 'use_legacy_attributes=False'
models = {
    'OLS - Basic': {
    ...
    'L1 (Lasso / Logistic L1)': {
        ...
        'classifier': make_pipeline(StandardScaler(), LogisticRegressionCV(cv=5, l1_ratios=[1.0], solver='saga', scoring='neg_log_loss', use_legacy_attributes=False, random_state=42, max_iter=10000, n_jobs=1))
    },
    'L2 (Ridge / Logistic L2)': {
        'regressor': make_pipeline(StandardScaler(), RidgeCV(cv=5)),
        'classifier': make_pipeline(StandardScaler(), LogisticRegressionCV(cv=5, l1_ratios=[0.0], solver='saga', scoring='neg_log_loss', use_legacy_attributes=False, random_state=42, max_iter=10000, n_jobs=1))
    },
    'Elastic Net': {
        'regressor': make_pipeline(StandardScaler(), ElasticNetCV(cv=5, l1_ratio=[0.1, 0.5, 0.9], random_state=42, max_iter=10000, n_jobs=1)),
        'classifier': make_pipeline(StandardScaler(), LogisticRegressionCV(cv=5, l1_ratios=[0.1, 0.5, 0.9], solver='saga', scoring='neg_log_loss', use_legacy_attributes=False, random_state=42, max_iter=10000, n_jobs=1))
    },
    'Single Tree (Depth 5)': {
    ...
    'Ensemble (Stacking)': {
        ...
        'classifier': StackingClassifier(estimators=base_classifiers, final_estimator=make_pipeline(StandardScaler(), LogisticRegressionCV(cv=5, l1_ratios=[0.0], solver='saga', scoring='neg_log_loss', use_legacy_attributes=False, max_iter=10000)))
    }
}
