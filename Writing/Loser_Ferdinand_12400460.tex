...
Table~\ref{tab:treatment_by_cluster} shows that these clusters both have a sufficient number of cities for both single policies, Congestion Pricing and Low-Emission Zones; however, the cities that implemented both are strongly concentrated in \textit{Cluster 1}. Hence, the GATT can be reliably estimated for both clusters with respect to the single treatment. However, for the synergy of both treatments, only the coefficient for \textit{Cluster 1} can be robustly estimated.

\section{Results}
\label{sec:results}

For all the estimations, AIPW and Causal forest find relatively similar point estimates, indicating that, if the assumptions hold, both methods come to essentially the same estimates. However, there is a striking difference in how much uncertainty is assessed for these coefficients. AIPW is much more conservative, often keeping the null hypothesis that the coefficients are statistically not different from 0. Whereas, Causal Forest calculates much smaller standard errors and finds that most estimates are significant. I  will first answer the three research questions based on the main results in Table~\ref{tab:dml_master_results} and then discuss possible concerns surrounding the magnitude of the estimates and their significance in Section~\ref{sec:sensitivity_analysis}.

\input{Tables/dml_master_results.tex}

\textbf{Research Question 1. } Congestion pricing as well as low-emission zones are estimated to have a negative effect on transport $CO_2$ emissions. Congestion Pricing is estimated to reduce emissions by 24 \% (AIPW) to 31 \% (Causal Forest). Low-Emission Zones are expected to lead to a reduction of 10 \% (AIPW) to 15 \% (Causal Forest). Yet, whether these results can be interpreted as significantly different from 0 (and not just caused by pure chance), depends entirely on which method is used. Causal Forest estimates small standard errorrs, determining that all estimates are significant to the 5 \% level. On the other hand, AIPW sees more uncertainty in its estimates, keeping the null hypothesis for CP and only finding signifance to the 10 \% level for LEZ.

\textbf{Research Question 2. } Panel B in Table~\ref{tab:dml_master_results} shows separate estimations for the GATT for both clusters. Across both policies (and also their synergy), point estimates are generally larger for \textit{Cluster 1}, the Dense Metropolis type, than for \textit{Cluster 0}, Sprawling Regional Hubs. AIPW actually does not find any significant effects for sinngle policies in \textit{Cluster 1}, yet does determine the effect of congestion pricing to be significantly negative in \textit{Cluster 0}. Therefore, although point estimates may be higher in \textit{Cluster 1}, AIPW only asserts significant efficiency of the congestion pricing polciy for sprawling hubs where the effect is determined to be nearly 10 \% lower. Meanwhile Causal Forest is again much more confident about all estimates with the exception of LEZ in \textit{Cluster 0} which has a p-value of nearly 0.4.

\textbf{Research Question 3. } The most startling result is definitely the coefficient for the synergy effects of implementing both CP and a LEZ simulataneously. Both AIPW and Causal Forest estimate that this combination of policies lead to a reduction of $CO_2$ in treated cities by more than 70 \%. This is a lot more than the simple addition of both policy's effects which is just 41 \% even for the larger coefficients estimated by Causal Forest. Therefore, the additional synergy effect can be determined to be around 30 \% by Causal Forest estimation and 40 \% by AIPW estimation. As established before, only \textit{Cluster 1} has a large enough sample of cities that implemented both policies to estimate this effect reliably. The effect size is approximately the same as for the ATT; which is reasonable as most cities with this treatment type are in \textit{Cluster 1}. All results on synergy effects are determined with significance to the 1 \% level, both for AIPW as well as for Causal Forest.  
